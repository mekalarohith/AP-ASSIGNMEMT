...
//To present Menu and to deal with different options from users.
	public void begin() {
		...
		try {
			...
			if (choice==2 & flag & flag2) {

				predictWinner();
			}
			if (choice==3 & flag & flag2) {
				flag2 = false;
				startGame();
			...
			if (choice==2& !flag || choice==3 & !flag || choice==4 & !flag
					|| choice==5 & !flag) {
				
				System.out.println("Please select a game to run firstly!!!");
				begin();
			} 
			if ((choice==2 & !flag2&choice!=6) || (choice==3 & !flag2&choice!=6)) {
				System.out.println((choice==2 & !flag2&choice!=6) || (choice==3 & !flag2&choice!=6));
...
//To display the final result of all games including the name of the referee for each game.
